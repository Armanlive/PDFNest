...
const PDFToWord = () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      const wordDoc = await DocumentProcessor.pdfToWord(files[0], setProgress);
      DocumentProcessor.downloadFile(wordDoc, `${files[0].name.replace('.pdf', '.docx')}`, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
    } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">Convert PDF documents to editable Word format</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <FileUpload
          ...
          description="Choose a PDF file to convert to Word document"
        />
